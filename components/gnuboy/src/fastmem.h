...
static byte IRAM_ATTR readb(int a)
{
...
static void IRAM_ATTR writeb(int a, byte b)
{
...
static int IRAM_ATTR readw(int a)
{
	if ((a+1) & 0xfff)
	{
		byte *p = mbc.rmap[a>>12];
...
static void IRAM_ATTR writew(int a, int w)
{
	if ((a+1) & 0xfff)
	{
		byte *p = mbc.wmap[a>>12];
...
static byte IRAM_ATTR readhi(int a)
{
	return readb(a | 0xff00);
}

static void IRAM_ATTR writehi(int a, byte b)
{
	writeb(a | 0xff00, b);
}
